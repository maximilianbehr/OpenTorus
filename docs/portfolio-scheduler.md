...
|---|---|---|
| prover | 5 chat turns, then the `proof_write` bootstrap → a scaffold sketch with a gap; each explicit gap becomes an obligation proposal (referee / formal / SMT / sympy closure modes); a gap-free sketch proposes a whole-proof obligation. Non-primary branches (special-case, obstruction, …) write *exploration* sketches with a bridge, never the dossier's primary answer. Later attempts run in gap-fill mode; no `proof_write` → `model_no_progress` |
...
| librarian | coverage assessment (≤ partial), `branch_done` | — |
