        ...
        try:
            if kind == "doi":
                ...
                try:
                    ...
                    record = None
                if record is None:
                    # "(untitled)" is what every other connector stores for a title it
                    # does not have. Synthesising "arXiv:{id}" instead put a string that
                    # reads as a title into the title field, which is the one thing the
                    # honesty rules forbid: missing metadata is marked missing.
